...
13. For ACC x and ACC z channels, select `<L1 or >L2, (Shock)` from the record limit drop-down menu
14. Set the corresponding `Limit L1` to -999 and `Limit L2` to +999. The MSR software will default to the maximum possible range
15. Make sure there are no limits set on the ACC y channel by selecting `inactive` from the record limit drop-down menu
16. Click the `<` button

![](figs/MSRStep4.png){fig-align='centre' width=70%}

17. Select the `Basic settings` tab
18. Under `Sensors`, select `t1` from the `ACC x, y, z` drop-down menu
19. Under `Main storage rate`, set `t1=` to the desired sampling rate in seconds
    * The Mini Buoy has been calibrated to accept sampling rates between 1 and 10 seconds. Refer to @tbl-1 for sampling duration (this is more accurate than the `Prediction` tool that underestimates memory capacity as it does not take into account the limit settings).
20. Under `Record control`, check the `Limits active` box
...
24. Click `Exit` and disconnect the MSR145W B4 logger from the computer. A blue light will begin to flash, confirming the logger is primed and ready to begin logging when the start date and time is reached
...
6. From the `Deployments` tab, set the `Logging Interval` to the desired sampling length using the drop-down menu.
    * The Mini Buoy has been calibrated to accept sampling rates between 1 and 10 minutes. Refer to @tbl-1 for sampling duration.
